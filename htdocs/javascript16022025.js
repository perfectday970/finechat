...
function startContinuousRecognition() {
  //enableEchoCancellation();  // Echo-Unterdrückung aktivieren
  ...
  recognition.onstart = function() {
    ...
    checkTimer = setInterval(checkSilence, 10);
  };
...
function playNextAudio() {
    if (isTTSRunning || audioQueue.length === 0) { // if (isTTSRunning || audioQueue.length === 0) {
...
function playAudio(audioData, sequenceId) {
    ...
    if (audioData.type === "browser") {
        ...
        // Für Coqui-TTS: Dekodiere die WAV-Datei und erstelle einen BufferSource
        fetch(audioData.data)
            .then(response => response.arrayBuffer())
            .then(arrayBuffer => audioContext.decodeAudioData(arrayBuffer))
            .then(audioBuffer => {
                const ttsBufferSource = audioContext.createBufferSource();
                ttsBufferSource.buffer = audioBuffer;

                // Erstelle einen DelayNode, um das TTS-Referenzsignal anzupassen
                const ttsDelay = audioContext.createDelay();
                ttsDelay.delayTime.value = 0.05; // z.B. 50 ms – experimentiere mit dem Wert!

                // Route den TTS-Stream zuerst über den DelayNode zum Filter (Eingang 1)
                ttsBufferSource.connect(ttsDelay);
                ttsDelay.connect(adaptiveFilterNode, 0, 1);

                // Gleichzeitig direkt an die Lautsprecher schicken, damit du es hörst
                ttsBufferSource.connect(audioContext.destination);

                ttsBufferSource.start();
                handleTTSStart();
                ttsBufferSource.onended = () => handleTTSEnd();
            })
            .catch(err => console.error("Fehler beim Decodieren des TTS AudioBuffer:", err));
    }
}
...
function enableEchoCancellation() {
    const constraints = {
        audio: {
            echoCancellation: true,   // Wichtig: Echo-Unterdrückung aktivieren
            noiseSuppression: true,   // Rauschunterdrückung für klarere Eingabe
            autoGainControl: true     // Vermeidet zu leise/lautes Mikrofon
        }
    };

    return navigator.mediaDevices.getUserMedia(constraints)
        .then((stream) => {
            microphoneStream = stream;  // Mikrofon-Stream speichern
            audioContext = new AudioContext();
            const source = audioContext.createMediaStreamSource(stream);
            source.connect(audioContext.destination);  // Verbinde das Mikrofon-Signal mit dem AudioContext
            console.log("Echo-Unterdrückung aktiviert.");
            return stream;
        })
        .catch((err) => {
            console.error("Fehler beim Aktivieren der Echo-Unterdrückung:", err);
            throw err;
        });
}*/
...
(async function initAudio() {
  // Erstelle den AudioContext und lade den Worklet-Processor
  window.audioContext = new AudioContext();
  await audioContext.audioWorklet.addModule('adaptive-filter-processor.js');

  // Hole das Mikrofonsignal
  const stream = await navigator.mediaDevices.getUserMedia({
    audio: { echoCancellation: true, noiseSuppression: true, autoGainControl: true }
  });
  const micSource = audioContext.createMediaStreamSource(stream);

    // Erstelle einen Gain Node, um das Mikrofonsignal zu dämpfen
  const micGain = audioContext.createGain();
  micGain.gain.value = 0.2;  // Passe den Wert nach Bedarf an – hier 20% des Originalpegels

  // Verbinde das Mikrofonsignal über den Gain Node
  micSource.connect(micGain);

  // Erstelle den adaptiven Filter-Worklet-Node mit 2 Eingängen:
  // Eingang 0: Mikrofonsignal, Eingang 1: TTS-Referenz (dynamisch aus playAudio)
  window.adaptiveFilterNode = new AudioWorkletNode(audioContext, 'adaptive-filter-processor', {
    numberOfInputs: 2,
    numberOfOutputs: 1,
    outputChannelCount: [1]
  });

  // Verbinde das Mikrofonsignal mit Eingang 0 des Filters
  micGain.connect(adaptiveFilterNode, 0, 0);

  // Verbinde den Ausgang des Filters mit den Lautsprechern
  adaptiveFilterNode.connect(audioContext.destination);

  // Es wird kein statischer TTS-Source mehr erstellt.
  // Die TTS-Audioquellen werden dynamisch in playAudio über adaptiveFilterNode (Eingang 1) eingespeist.
})();
